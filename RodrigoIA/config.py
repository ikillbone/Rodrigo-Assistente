...
version = "1.5.0"
cidade = 'santos'
caminho_navegador = r"C:\Program Files (x86)\Microsoft\Edge\Application\msedge.exe s%"

def intro():
	msg = "Rodrigo - version {} / by: Ryan & ALves - from BBT".format(version)
	print("-" * len(msg) +  "\n{}\n".format(msg)  +   "-" * len(msg))
...
def  verifica_nome_exist(nome):
	dados = open("RodrigoIA/dados/nomes.txt", "r")
	nome_list = dados.readlines()

	if not nome_list:
		vazio = open("RodrigoIA/dados/nomes.txt", "r")
		conteudo = vazio.readlines()
		conteudo.append("{}".format(nome))
		vazio = open("RodrigoIA/dados/nomes.txt", "w")
...
def name_list():
	try:
		nomes = open("RodrigoIA/dados/nomes.txt", "r")
		nomes.close()

	except FileNotFoundError:
		nomes = open("RodrigoIA/dados/nomes.txt", "w")
		nomes.close()
